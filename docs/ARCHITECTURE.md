...
- Edits mutate objects in the `edit` context. *Pending Changes* = `insertedObjects` / `updatedObjects` (with `changedValues()` vs `committedValues(forKeys:)`) / `deletedObjects`, exposed as `[PendingChange]`. (As built, M3-06: an inserted object has no `ObjectRef` until the commit gives it a primary key. `StoreSession.stagedObject(_:)` reads any object by its `PendingObjectID`, inserted ones included, and `CommitSummary.insertedRefs` gives the reference each inserted object got, so a front end showing one goes on showing it. Relationships are edited with `link`, `unlink` and `insertRelatedObject`, each one undoable edit whose inverse Core Data keeps; `RelatedObjects.Item` names its object by `PendingObjectID`, so a linked object only inserted is listed as well.)
