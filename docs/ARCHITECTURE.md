...
public enum Value: Sendable, Hashable, Codable {
    ...
    case toOneInserted(PendingObjectID, display: String?)     // as built, M3-06: led to an object only inserted
...
| Editable | no read-only option; migration and inference off, said explicitly; `NSPersistentHistoryTrackingKey = true` **iff** `probe.hasHistory`; `journal_mode = DELETE` iff the header says rollback journal | Never turn history on for a store that lacks it (EDT-5). Transaction author `CoreDataDabbi` on every context. Core Data puts any store it opens read-write into WAL mode; the pragma keeps the app's choice (M3-01). Refused with `.storeNotWritable` — checked before Core Data, which would fail only at the first save — when the file, its folder or an existing `-wal`/`-shm` cannot be written; never answered with a working copy. A save through the sanitised model leaves `Z_MODELCACHE` byte-identical. |
...
- Edits mutate objects in the `edit` context. *Pending Changes* = `insertedObjects` / `updatedObjects` (with `changedValues()` vs `committedValues(forKeys:)`) / `deletedObjects`, exposed as `[PendingChange]`. (As built, M3-06: an inserted object has no `ObjectRef` until the commit gives it a primary key. `StoreSession.stagedObject(_:)` reads any object by its `PendingObjectID`, inserted ones included, and `CommitSummary.insertedRefs` gives the reference each inserted object got, so a front end showing one goes on showing it. Relationships are edited with `link`, `unlink` and `insertRelatedObject`, each one undoable edit whose inverse Core Data keeps; `RelatedObjects.Item` names its object by `PendingObjectID`, so a linked object only inserted is listed as well. A to-one that leads to an object only inserted reads as `Value.toOneInserted`, naming it by the identity it was staged under, and reads as `.toOne` once the commit gives it a reference; `setValue` takes either.)
- Undo/redo = the context's `UndoManager`, bridged to the window's undo manager. (As built, M3-04: the edit context's manager does not group by event — one group per staged edit — and its depth is reported with every answer as `PendingChanges.undoDepth`; the window's manager registers one mirrored entry only when that depth grew, as a top-level group of its own, since answers arrive in task continuations that would otherwise share one event group.)
- After every staged edit the touched objects run `validateForInsert/Update/Delete`; `NSError`s (incl. `NSDetailedErrorsKey`) are mapped by `ValidationTranslator` to per-field, plain-language `ValidationIssue`s (EDT-2). (As built, M3-05: every changed object is validated whenever what is staged is reported — each edit, undo and redo — and the issues travel as `PendingChanges.issues`; the rule comes from the error code when it names one and otherwise from the shape of the failed predicate, `NSValidationPredicateErrorKey`, since a model built in code reports the generic code; the value, `NSValidationValueErrorKey`, is never read. A refused commit's diagnosis is the same issues. `StoreSession.deletePreview(of:)` follows the delete rules by reading rather than by staging and undoing, so that a cancelled preview leaves the redo stack alone, and finds what a relationship without an inverse leaves dangling by a fetch.)
...
**Live-process guard**: `proc_listpidspath` (libproc) lists PIDs that have the store file open — precise, and works for simulator apps because they are host processes. Offers `simctl terminate <udid> <bundle-id>`. (As built, M3-03: `LiveProcesses` in `DabbiSnapshots` checks the database, `-wal` and `-shm` and excludes this process; the call walks every process's descriptors, ~85 ms per path, so it is never made on the main thread. A Mac app is asked with `NSRunningApplication.terminate()`; any other process is named and left to the user.)
...
- **Snapshots:** backup API copy from a read-only connection + external-data folder (`.<store>_SUPPORT/_EXTERNAL_DATA`), manifest with name/note/model hashes. The same code performs the pre-commit backup (EDT-9). (Amended in M3-02: the support folder is copied before and after the database, which is enough because external-data files are only ever added or removed, never rewritten; a copy is built in a staging folder and renamed into place once verified; the row-count comparison with the live store is made only when `data_version` shows no other commit in between, and the manifest records whether it was; backups live per store under `~/Library/Application Support/CoreDataDabbi/Backups/<NSStoreUUID>/` with a retention that never removes user snapshots or the newest backup.) **Restore:** live-process guard → remove `-wal`/`-shm` → atomic replace → re-open. (As built, M3-03: `Restorer` verifies the snapshot against its manifest, refuses a held store (`snapshot.storeInUse`), backs the store up into the same library, stages the copy in the store's own folder so the swap is a rename on one volume, checks the holders again, parks `-wal`/`-shm` — a log left behind would be replayed into the restored database — `replaceItemAt`s the database (the side files go back on failure), swaps the support folder, and removes the staging; failures are `snapshot.restoreFailed`. The app closes its session first and reopens after, through `ProjectContext.openStore(preparing:)`. User snapshots share the per-store backup library rather than living in the project package, which NSDocument rewrites on every save.) **Diff:** compatible-model check → per-entity PK sets and `Z_OPT`/row hashes through two raw connections → materialise differing rows through Core Data for field-level output.
...
- ADR-12 in practice: `WriteAuthorization` has a `package`-level initialiser wrapped by two public factories that live in code the MCP command does not link against; a test asserts the MCP tool list contains no mutating tool. As of M3-01 only the app's factory exists (`WriteAuthorization.app`, in `DabbiKit`); the CLI's comes with `--allow-writes`, and both move out of the MCP server's reach when it lands.
