...
extension ProjectContext {
    ...
    func editableAttribute(_ name: String, of ref: ObjectRef) -> AttributeDescription? {
        guard accessMode == .editable,
            let attribute = model?.entity(named: ref.entity)?.attribute(named: name),
            !attribute.isTransient, !attribute.isDerived, ValueText.isEditableAsText(attribute.type)
        else { return nil }
        return attribute
    }

    /// Stages what was typed for `attribute` of `ref`, read in the project's time zone.
    ///
    ...
    func stage(_ text: String, for attribute: AttributeDescription, of ref: ObjectRef) -> String? {
        do {
            ...
            editing.setValue(value, for: attribute.name, of: PendingObjectID(ref))
            return nil
        } catch {
    ...
    func clear(_ attribute: AttributeDescription, of ref: ObjectRef) {
        editing.setValue(.null, for: attribute.name, of: PendingObjectID(ref))
    }

    /// How the property `name` of `ref`, which holds `value` now, is edited — `nil` when it cannot be typed.
    func fieldEditing(_ name: String, value: Value, of ref: ObjectRef) -> FieldEditing? {
        guard let attribute = editableAttribute(name, of: ref) else { return nil }
        var clear: (@MainActor () -> Void)?
        if attribute.isOptional, !value.isNull {
            clear = { [weak self] in self?.clear(attribute, of: ref) }
        }
        return FieldEditing(
            text: ValueText.text(for: value, timeZone: timeZone),
            stage: { [weak self] in self?.stage($0, for: attribute, of: ref) }, clear: clear)
    }
}
